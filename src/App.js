...
import { BRAND_NAME, BRAND_REPO_URL, BRAND_TAILWIND_CDN } from './brand';
...
const themeList = ['teal', 'emerald', 'cyan', 'amber', 'rose', 'sky', 'lime'];
...
class App extends Component {
  constructor(props) {
    ...
    this.handleContentDidUpdate = this.handleContentDidUpdate.bind(this);
  ...
  ensureTailwindRuntime(frameDocument) {
    if (!frameDocument || frameDocument.getElementById('tailwind-runtime')) {
      return;
    }

    const runtimeScript = frameDocument.createElement('script');
    runtimeScript.id = 'tailwind-runtime';
    runtimeScript.src = BRAND_TAILWIND_CDN;
    frameDocument.head.appendChild(runtimeScript);
  }
    ...
    return (
      <div className={`app${darkMode ? ' dark-mode' : ''}${sidebar ? ' has-sidebar' : ''} ${theme} ${view}`}>
        ...
        <div className="toolbar">
          <button className="opener" onClick={this.toggleSidebar} ref={this.openerRef}>{BRAND_NAME}</button>
          {this.state.codeView &&
        ...
        <a href={BRAND_REPO_URL} className="github" target="_blank" rel="noopener noreferrer">
